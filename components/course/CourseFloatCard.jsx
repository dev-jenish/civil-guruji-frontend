...
export default function CourseFloatCard({
  isPackage,
  courseData,
  totalLearningHours,
}) {
  ...
  if (
    courseData?.courseDetail?.courseContents &&
    courseData?.courseDetail?.courseContents?.length > 0
  ) {
    ...
    if (
      ...
      userData.purchases.forEach((plan) => {
        ...
        if (
          ...
          plan.emisPaid < 3 &&
          plan.emisPaid != null &&
          currentTime.isBefore(moment(plan.validityDate))
        ) {
  ...
  return (
    <div
      ...
      style={isPackage ? { marginTop: "-12rem" } : {}}
    >
            ...
            {displayPlan && Object.keys(displayPlan)?.length > 0 ? (
              ...
              >
                <div className={styles.singleBox}>
                  <div className={styles.pricePart}>
                    <Text className={styles.afterDiscountPrice}>
                      ₹{`${displayPlan?.discountedPrice || "Free"}`}
                    </Text>
                  ...
                  >
                    <Text className={styles.validity}>
                      Validity :{" "}
                      {displayPlan?.validity == "lifetime"
                        ? "Lifetime"
                        : displayPlan?.validity == "limited"
                        ? `${
                            displayPlan?.validityYears > 0
                              ? displayPlan?.validityYears + " Years"
                              : ""
                          } ${
                            displayPlan?.validityMonths > 0
                              ? displayPlan?.validityMonths + " Months"
                              : ""
                          }`
                        : "Error"}
                    </Text>
                ...
                <VStack justifyContent="flex-end" paddingTop="10px">
                  ...
                  >
                    {emiPurchased.includes(displayPlan?._id)
                      ? "Pay EMI"
                              ...
                              >
                                  ...
                                  >
                                    <Text className={styles.validity}>
                                      ...
                                      {plan?.validity == "lifetime"
                                        ? "Lifetime"
                                        : plan?.validity == "limited"
                                        ? `${
                                            plan?.validityYears > 0
                                              ? plan?.validityYears + " Years"
                                              : ""
                                          } ${
                                            plan?.validityMonths > 0
                                ...
                                >
                                  <Button
                                    ...
                                    onClick={() =>
                                      router.push(
                                        `/checkout/${courseData?._id}/${plan?._id}`
                                      )
                                    }
                                    borderRadius={2}
                                    size="sm"
                                  ...
                                  >
                                    {emiPurchased.includes(plan?._id)
                                      ? "Pay EMI"
                                      : purchased.includes(plan?._id)
                                      ? "Already purchased"
                                      : "Get Offer"}
                                  </Button>
                                </HStack>
